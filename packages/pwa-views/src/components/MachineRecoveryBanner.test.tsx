...
describe('MachineRecoveryBanner', () => {
  ...
  it('makes reliable-rate fallback explicit and offers a provider-neutral retry', () => {
    ...
    expect(screen.getByText(/configured availability threshold/)).toBeInTheDocument()
    expect(screen.queryByText(/five-minute/)).not.toBeInTheDocument()
    expect(screen.queryByText(/same zone/)).not.toBeInTheDocument()

...
func TestLoadHermesCatalogReadsTheActiveProvider(t *testing.T) {
	...
	models, err := LoadHermesCatalog(provider, metadata, []string{"kyber-endpoint"}, nil, 100)
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func TestLoadHermesCatalogStillEnrichesOpenRouter(t *testing.T) {
	...
	models, err := LoadHermesCatalog(provider, metadata, []string{"openrouter"}, nil, 100)
	if err != nil {
		t.Fatalf("load: %v", err)
	}
	if len(models) != 1 {
...
func TestLoadHermesCatalogDefaultsToOpenRouter(t *testing.T) {
	...
	models, err := LoadHermesCatalog(provider, metadata, []string{""}, nil, 100)
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func TestLoadHermesCatalogToleratesMissingMetadataForACustomProvider(t *testing.T) {
	...
	models, err := LoadHermesCatalog(providerPath, filepath.Join(dir, "absent.json"), []string{"kyber-endpoint"}, nil, 100)
	if err != nil {
...
func TestLoadHermesCatalogStillErrorsOnMissingOpenRouterMetadata(t *testing.T) {
	...
	_, err := LoadHermesCatalog(providerPath, filepath.Join(dir, "absent.json"), []string{"openrouter"}, nil, 100)
	if err == nil {

...
func (s *PostgresStore) Migrate(ctx context.Context) error {
	...
	statements := []string{
		...
		`DO $$ DECLARE c record; BEGIN FOR c IN SELECT conname FROM pg_constraint WHERE conrelid='agent_tasks'::regclass AND contype='c' AND pg_get_constraintdef(oid) LIKE '%state IN (%' LOOP EXECUTE format('ALTER TABLE agent_tasks DROP CONSTRAINT %I', c.conname); END LOOP; ALTER TABLE agent_tasks ADD CONSTRAINT agent_tasks_state_check CHECK (state IN ('queued','dispatched','canceling','canceled','completed','failed')); EXCEPTION WHEN duplicate_object THEN NULL; END $$`,

...
func (s *PostgresStore) Reconcile(ctx context.Context, limit int) (*ReconcileResult, error) {
	...
	if _, err := tx.ExecContext(ctx, `WITH expired AS (SELECT id FROM agent_tasks WHERE state IN ('canceled','completed','failed') AND retain_until<clock_timestamp() LIMIT $1) UPDATE agent_task_objects o SET status='deleting' FROM expired WHERE o.task_id=expired.id AND o.status='ready'`, limit); err != nil {
		return nil, err
	}
	queries := []struct {
		q string

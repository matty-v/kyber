...
Token usage is written via `POST /internal/agents/{name}/token-usage`. The handler computes the per-report delta and writes it to **both** Tier-1 sources: `tokenAccumulator.IncrBy(accum:token_usage:{ns}:{agent}:{model})` for the all-time totals, and `MetricsStore.AddPoint(ts:token_usage:{ns}:{agent}:{model}:{type})` for each non-zero type component (`input`/`cache_creation`/`cache_read`/`output`) so the windowed read can honor `start`/`end` (kyber#428). The windowed series stores deltas and is summed over the window, identical to the activity series. All four types are quasi-cumulative counters flowing through the same `safeDelta` logic: `output` is the reporter-accumulated total (since reporter start for Claude Code via the in-pod `outputTracker`; since rollout-session start for Codex via `total_token_usage`), and a counter rollback (reporter/session restart) is treated as a fresh increment. The `output` type was added later than the other three; for up to 7 days after that deploy (retention == max query window) mixed windows under-report output for their pre-upgrade portion — never worse than the previous always-zero — and the window self-heals as retention rolls over.

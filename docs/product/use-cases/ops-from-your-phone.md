...
Assign work, approve decisions, and get results over Telegram or Discord, without opening a laptop.
...
Plenty of tools can page you. The difference here is that the thing messaging you can also do the work. Channels are two-way: an inbound message lands as the agent's next prompt, and the agent replies from inside the pod where the work actually happened. A Telegram message can even wake an agent that was suspended to save resources; Discord needs the agent's pod running to receive.
...
- [Chat: Telegram and Discord](../capabilities/chat-channels.md): how channels bind to agents.
- [Quickstart](../getting-started/quickstart.md): get a fleet console running first.

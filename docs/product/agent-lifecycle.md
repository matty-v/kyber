...
| `NeedsAuth` | The agent's stored authorization is no longer valid; it can't work until re-authorized. Kyber reaches this automatically when an agent's authorization expires, and an operator can also **force** an agent here to recover it from a wedged state (see Operator actions). | **Re-authorize it** — or, if you have already supplied a working credential and the agent is still parked, **restart its pod** (see Operator actions). |
...
| Restart a `NeedsAuth` agent's pod | PWA → Agents → Lifecycle menu → **Restart pod** (confirmation required) | Rebuilds the pod using the credentials the agent already has, and the agent returns to `Starting` → `Running`. This is the control for the case where a working credential was already supplied but the agent stayed parked — Kyber only retries a human-required phase by itself once the supplied credential actually changes, so an operator who re-supplied the *same* good credential needs a way to say "try again anyway". Each click is one rebuild attempt: if the credential really is bad the agent lands back in `NeedsAuth` rather than looping, and Re-authorize is then the right action. |

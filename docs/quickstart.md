...
GitOps tooling, no fork.

This is the shortest honest path. When you want a permanent install with a real
public URL and cloud compute, continue to [installation.md](./installation.md)
(GCP) or [installation-wsl2.md](./installation-wsl2.md) (a Windows laptop) —
both start from the same `helm install` you run here.
...
**What this does:** creates `kyber-system` and the three secrets Kyber needs
before it starts. All three are generated locally — nothing is fetched, and
nothing here is committed anywhere.

```bash
kubectl create namespace kyber-system --dry-run=client -o yaml | kubectl apply -f -

# Keep the API key. It unlocks every /api/v1/* endpoint — treat it as a root
...
in place of `1.0.7`; the chart version and the release tag are the same number.
...
attaches to the Kubernetes node you already have — creating one provisions
...
| You want | Read |
|---|---|
| A permanent install on GCP | [installation.md](./installation.md) |
| A permanent install on a Windows laptop | [installation-wsl2.md](./installation-wsl2.md) |
| A public HTTPS URL (needed for Telegram) | [installation.md § 11](./installation.md#11-https-via-tailscale-funnel) |
...
| Hack on Kyber itself | [`scripts/devenv/full-local.md`](../scripts/devenv/full-local.md) |
...
Agents still exist and you remove the very controller that would release them —
the namespace then hangs in `Terminating` forever:

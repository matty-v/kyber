...
**Verify:**
```bash
helm show chart oci://ghcr.io/matty-v/charts/kyber --version 1.4.0 \
  | grep -E '^(name|version):'
helm show values oci://ghcr.io/matty-v/charts/kyber --version 1.4.0 \
  | grep -c 'tag: "v1.4.0"'
```
Expected output: `name: kyber` and `version: 1.4.0`, then `8` — the eight container images the chart pins for you.
...
Use the newest version from https://github.com/matty-v/kyber/releases in place of `1.4.0` — the chart version and the release tag are the same number.

...
# Prove the harness itself works before asking it about credentials. A broken
...
if [ "$_runtime_probe_status" -ne 0 ] || [ -z "$_runtime_probe_version" ]; then
    ...
    curl -fsS --max-time 5 --retry 5 --retry-connrefused -H 'Content-Type: application/json' -X POST \
        -d "{\"version\":\"unknown\",\"runtime\":\"codex\",\"usable\":false,\"probeMessage\":\"${_runtime_probe_json}\"}" \
        http://127.0.0.1:8091/runtime-version >/dev/null 2>&1 || true
    echo "[kyber] FATAL: Codex runtime probe failed: $_runtime_probe_message" >&2
    exit 43
fi
curl -fsS --max-time 5 --retry 5 --retry-connrefused -H 'Content-Type: application/json' -X POST \
...
if [ "$KYBER_CODEX_CONFIG_USABLE" = "true" ]; then
    ...
    kyber_converge_mcp kyber_slack "${KYBER_SLACK_MCP_URL:-}" "Slack"

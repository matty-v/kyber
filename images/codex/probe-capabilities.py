...
print(json.dumps({'job-turn-hooks':has('UserPromptSubmit',start) and has('Stop',stop) and pathlib.Path(os.environ.get('KYBER_CRON_POSTRUN_SENTINEL','/persist/var/run/kyber-cron-postrun-enabled')).exists(), 'task-receipts':has('UserPromptSubmit',receipt), 'task-tools':bool(os.environ.get('KYBER_REQUEST_MCP_URL')) and server.get('url')==os.environ.get('KYBER_REQUEST_MCP_URL') and server.get('enabled',True) is True}))
